...
| 端口 | 服务 | 说明 |
|------|------|------|
| `:27041` | **Client API** | 面向 ClassIsland 客户端 — 提供配置清单（Manifest）和资源文件下载 |
| `:27042` | **Management API** | 用户管理 — 认证、账户操作、资源管理、客户端控制 |
| `:27043` | **Admin API** | 超管后台 — 全局用户/账户管理、系统设置 |
| `:27044` | **gRPC** | Cyrene_MSP 协议 — 客户端注册、命令推送、配置上传、审计 |
...
| `CIMS_CLIENT_PORT` | `27041` | Client API 端口 |
| `CIMS_MANAGEMENT_PORT` | `27042` | Management API 端口 |
| `CIMS_ADMIN_PORT` | `27043` | Admin API 端口 |
| `CIMS_GRPC_PORT` | `27044` | gRPC 服务端口 |
...
├── app/
│   ├── api/                        # FastAPI 路由层
│   │   ├── admin/                  #   超管 API（用户/账户/设置/批量）
│   │   ├── client/                 #   客户端 Manifest & 资源下载
│   │   ├── command/                #   指令下发 & 配置文件管理
│   │   ├── management/             #   管理 API（认证/账户/资源/客户端）
...
│   │   ├── management_app.py       #   Management API 应用
