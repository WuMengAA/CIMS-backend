from sqlalchemy import Column, Integer, String, JSON, Float, Boolean, ForeignKey, UniqueConstraint
...
class Tenant(Base):
    ...
    client_statuses = relationship("ClientStatus", back_populates="tenant")
    profile_configs = relationship("ProfileConfig", back_populates="tenant")
    pre_registers = relationship("PreRegister", back_populates="tenant")
...
class ClientStatus(Base):
    ...
    __table_args__ = (UniqueConstraint('tenant_id', 'uid', name='_tenant_status_uid_uc'),)
...
class ProfileConfig(Base):
    ...
    __table_args__ = (UniqueConstraint('tenant_id', 'uid', name='_tenant_profile_uid_uc'),)
...
class PreRegister(Base):
    ...
    __table_args__ = (UniqueConstraint('tenant_id', 'client_id', name='_tenant_preregister_client_id_uc'),)
