...
{time_during:>120}"""
    ...
    def log(self, message: str | RichText, status: str | RichText):
        ...
        message += " "
        try:
            self.logger_file = open(
                "logs/{}.log".format(datetime.now().strftime("%Y-%m-%d %H")), "a"
            )
        except FileNotFoundError:
            project_info = json.load(open("project_info.json", "r"))
            self.logger_file = open(
                "logs/{}.log".format(datetime.now().strftime("%Y-%m-%d %H")), "w"
            )
        ...
        if len(message) < 91:
            ...
            self.logger_file.write(
                "[{}] {}\n".format(
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"), message[0:91]
                )
            )
            ...
            if len(message[(int(len(message) / 91)) * 91 :]) < 91:
                if len(message[(int(len(message) / 91)) * 91 :] + status_message) > 91:
                    self.logger_file.write(
                        "{}{}\n".format(
                            " " * 29, message[(int(len(message) / 91)) * 91 :]
                        )
                    )
                    self.logger_file.write(
                        "{}{}\n".format(
                            " " * (120 - len(status_message)), status_message
                        )
                    )
                else:
                    self.logger_file.write(
                        "{}{}{}{}\n".format(
                            ...
                            * (
                                91
                                - len(status_message)
                                - len(message[(int(len(message) / 91)) * 91 :])
                            ),
                            status_message,
                        )
                    )
