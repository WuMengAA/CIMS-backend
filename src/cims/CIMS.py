...
def main():
    ...
    for _file in (
        ["./settings.json"]
        + [
            "./Datas/{}.json".format(name)
            for name in ["client_status", "clients", "pre_register", "profile_config"]
        ]
        + [
            "./Datas/{}/default.json".format(name)
            for name in [
                "ClassPlan",
                "DefaultSettings",
                "Policy",
    ...
    if installed:
        ...
        for part in ["gRPC", "api", "command"]:
            _input = input(
                ...
                "(Enter directly to use default settings):".format(
                    part=part,
            ...
            while _part_set:
                try:
                    if _input.startswith("http://"):
                        print(
                            (
                                "HTTP is not safe and HTTPS recommended.\n"
                                if not _input.startswith("http://localhost")
                                else ""
                            ),
                            end="",
                        )
            ...
            if _input != "":
                _input = input(
                    "{part} listening port(Enter directly to use the same as above):".format(
                        part=part
                    )
                )
                _part_set = True
        ...
        _set["organization_name"] = (
            _input if _input != "" else "CIMS Default Organization"
        )
